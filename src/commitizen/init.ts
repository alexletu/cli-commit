...
function init(repoPath, adapterNpmName, {
  ...
  const adapterConfig = loadAdapterConfig(repoPath);

  // Get the npm string mappings based on the arguments provided
  const stringMappings = yarn ? getYarnAddStringMappings(dev, exact, force) : getNpmInstallStringMappings(save, saveDev, saveExact, force);

  // Generate a string that represents the npm install command
  const installAdapterCommand = yarn ? generateYarnAddAdapterCommand(stringMappings, adapterNpmName) : generateNpmInstallAdapterCommand(stringMappings, adapterNpmName);

  const installCommitizenCommand = yarn ? generateYarnAddAdapterCommand(stringMappings, 'commitizen') : generateNpmInstallAdapterCommand(stringMappings, 'commitizen');
 ...
 */
function checkRequiredArguments(p, adapterNpmName) {
  if (!p) {
...
function loadAdapterConfig(cwd) {
  const config = configLoader.load(null, cwd);
  if (config) {
    return config;
  }
  return {};
