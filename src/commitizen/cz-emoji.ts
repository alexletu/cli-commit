...
async function getConfig() {
  ...
  const config =    (await loadConfigUpwards('package.json'))
...
function getEmojiChoices({ symbol }: { symbol: boolean }) {
...
function createQuestions(config) {
  ...
  const questions = [
    ...
    },
    {
      ...
      default  : (answers) => answers.type.description,
    },
    {
      type   : 'input',
      name   : 'body',
...
function format(answers) {
  ...
  const breaking =    answers.breakingBody && answers.breakingBody.trim().length !== 0
